...
Form.prototype.parse = function(req, cb) {
  ...
  if (cb) {
    ...
    // handleFiled 方法 emit 广播的 字段名 和 值
    self.on('field', function(name, value) {
      var fieldsArray = fields[name] || (fields[name] = []);
      fieldsArray.push(value);
    });
    // handleFile 方法 emit 广播的 字段名 和 值
    self.on('file', function(name, file) {
      var filesArray = files[name] || (files[name] = []);
      filesArray.push(file);
    });
    //接收到 外部 广播的 结束事件  在下一个 tick 执行 end 事件  (116 行)
...
//TODO: 解析流 的字段 
/**
 * @param fieldStream 这里 就是 stream 模块 fieldStream PassThrough 的实例
 * fieldStream = new stream.PassThrough()
 */
function handleField(self, fieldStream) {
  var value = '';      //保存读取到的buffer
  ...
  //读取结束后 emit 广播一个 filed 消息 Form.parse 方面里面 self.on('filed') 就能拿到了, 是一个消息发布订阅模式
